...
export default function Categories() {
    const results = [
      ...
      return (
        <div className="m-2 sm:m-5 bg-white rounded-lg mb-16">
          <div className="mt-5">
            <div className="grid grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 gap-0.5 p-4 text-center">
              {results.map((result, index) => (
                <div key={index}>
                  <CategoryCard key={index} result={result} />
                </div>
              ))}
            </div>
          </div>
